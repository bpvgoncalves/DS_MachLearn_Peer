...
There are two files with `r dim(trainData)[2]` variables and `r dim(trainData)[1]` records for the training dataset and `r dim(testData)[1]` records for the test cases to be submited. We'll keep the test cases apart until the prediction phase, later in this document. 
...
The next step is split the training dataset into 3 different sets: one for model training, another for model testing and a last one for model validation. About 60% of the samples will be used for training, and then 20% for each of testing and validation sets.

```{r DatasetSpliting}
trn_rows <- createDataPartition(trainClean$classe, p=0.10, list=FALSE)
...
confusionMatrix(testSet$classe, p1)
confusionMatrix(testSet$classe, p2)
```
