...
Once we finished with the previous steps and we have our final model we can then preform the last final step: use this final model to predict the 20 quiz cases.  
...
After submision, it resulted in 20 right predictions out of 20 records to predict. 
Playing arround with the Poisson distribution, an assuming an average 'miss ratio' of 1.5 pedictions out of every 100 attempts, we can compute a `r ppois(0, 20*1.5/100, lower=TRUE) ` probability of the 20/20 performance achieved (ie, 0 misses in 20 attempts).  
<br>
