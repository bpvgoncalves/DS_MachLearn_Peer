...
At the end, the outputs of all three models will be combined in a fourth model, trying to get the best of each of the initial models. A **Linear Discriminant Analysis (LDA)** model will be used here.  
<br>
...
The second level of cross-validation is preformed using the fitted models to predict the (known) class for the 'Testing Set' records and compare the prediction  with the actual class. The results, also measured by Accuracy, are the following: 

``` {r TestingSetError, fig.height=4, fig.width=10}
...
hist(rsamples$values[[2]], breaks=seq(0.85, 1, len=15), col="red", main="KNN", xlab="Accuracy")
hist(rsamples$values[[4]], breaks=seq(0.85, 1, len=15), col="blue", main="RF", xlab="Accuracy")
hist(rsamples$values[[6]], breaks=seq(0.85, 1, len=15), col="gold2", main="QDA", xlab="Accuracy")
hist(rsamples$values[[8]], breaks=seq(0.85, 1, len=15), col="forestgreen", main="Comb", xlab="Accuracy")
```

**Fig.1** - Distribution of each model Accuracy computed during the fitting process.  

We can see the Accuracy obtained on the 'Testing Set' is very similar to the mean and median expected Accuracy computed during model fitting. It seems models are not over-fitted to the training data and can 'safely' be used to predict classes on new datasets.  
<br>
